...
def _get_args():
    ...
    parser.add_argument("--num_words_th", default=256, type=int)
...
def _prepro(args):
    ...
    num_words_th = args.num_words_th
    ...
    _insert_raw_data(train_path, train_shared, train_batched, num_words_th=num_words_th)
    _insert_raw_data(dev_path, dev_shared, dev_batched, X_offset=len(train_shared['X']), num_words_th=num_words_th)
...
def _get_char2idx_dict(train_shared, train_batched, char_count_th=500):
    chars = [NULL, UNK]
...
def _tokenize(raw):
    ...
    tokens = list(itertools.chain(*[nltk.word_tokenize(sent) for sent in sents]))
    return tokens
...
def _insert_raw_data(file_path, raw_shared, raw_batched, X_offset=0, num_words_th=256, ques_num_words_th=64):
    START = "sstartt"
    STOP = "sstopp"
    X = raw_shared['X']
    ...
    with open(file_path, 'r') as fh:
        ...
        for article_idx, article in enumerate(tqdm(d['data'])):
            ...
            for para in article['paragraphs']:
                ...
                if len(words) > num_words_th:
                    logging.debug("Skipping para with num words = {}".format(num_words_th))
                ...
                for qa in para['qas']:
                    ...
                    if len(question_words) > ques_num_words_th:
                    ...
                    for answer in qa['answers']:
                        ...
                        temp_context = "{} {} {} {} {}".format(context[:answer_start], START,
                                                               ...
                                                               context[answer_stop:])
                        temp_words = _tokenize(temp_context)
                        start_idx = temp_words.index(START)
                        stop_idx = temp_words.index(STOP) - 1
                        if words[start_idx:stop_idx] != answer_words:
                            mismatch_count += 1
                ...
                # break  # for debugging
        if mismatch_count > 0:
            logging.warning("# answer mismatches: {}".format(mismatch_count))
        ...
        plt.show()
        num_words_counter = Counter(len(para) for paras in X for para in paras)
        plt.plot(list(num_words_counter.keys()), list(num_words_counter.values()))
        plt.show()
        """
